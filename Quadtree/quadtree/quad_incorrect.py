...
def defaultCollision(p, pt):
    """Default collision between two circles."""
    return (p[X] - p[RADIUS] <= pt[X] <= p[X] + p[RADIUS]) and (p[Y] - p[RADIUS] <= pt[Y] <= p[Y] + p[RADIUS])
